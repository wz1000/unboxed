...
instance EqRep WordRep where
  eqDef x y = not (x /= y)
  neDef x y = not (x == y)

instance OrdRep WordRep where
  compareDef x y
    | x == y = EQ
    | x <= y = LT
    | otherwise = GT
...
instance FractionalRep WordRep where
  fractionalDef x y = x * recip y
  recipDef x = 1 / x

instance RealRep WordRep where
  realToFracDef x = fromRational (toRational x)

instance EnumRep WordRep where
{-
...
instance IntegralRep WordRep where
...
instance RealFracRep WordRep where
...
instance FloatingRep WordRep where
  {-# INLINE powDef #-}
  {-# INLINE logBaseDef #-}
  {-# INLINE sqrtDef #-}
  {-# INLINE tanDef #-}
...
instance RealFloatRep WordRep where
  exponentDef x
    | m == 0 = 0
...
data instance ListD (a :: TYPE WordRep) = Nil | a :# List a
infixr 5 :#

instance ListRep WordRep where
  cons = (:#) 
  cons' a as = a :# as
  nil = Nil
...
instance Eq a => Prelude.Eq (ListD @WordRep a) where
  Nil == Nil = True
  a :# as == b :# bs = a == b && as == bs
  _ == _ = False
...
instance Ord a => Prelude.Ord (ListD @WordRep a) where
  compare Nil Nil = EQ
  compare Nil (:#){} = LT
  compare (:#){} Nil = GT
  compare (a:#as) (b:#bs) = compare a b <> compare as bs

instance ShowList a => Prelude.Show (ListD @WordRep a) where
  showsPrec _ = showList

data instance MaybeD (a :: TYPE WordRep) = Nothing | Just a

instance Eq a => Prelude.Eq (MaybeD @WordRep a) where
  Nothing == Nothing = True
  Just a == Just b = a == b
  _ == _ = False
  
instance Ord a => Prelude.Ord (MaybeD @WordRep a) where
  compare Nothing  Nothing = EQ
  compare Nothing  Just{} = LT
  compare Just{}   Nothing = GT
  compare (Just a) (Just b) = compare a b

instance Show a => Prelude.Show (MaybeD @WordRep a) where
  showsPrec _ Nothing = showString "Nothing"
  showsPrec d (Just a) = showParen (d >= 11) $ showString "Just " . showsPrec 11 a

instance MaybeRep WordRep where
  nothing = Nothing
  just = Just
  just' x = Just x
  maybe n _ Nothing = n
...
pattern Nothing# :: forall (a :: TYPE WordRep). Maybe# a
pattern Nothing# = Maybe# (# (##) | #)

pattern Just# :: forall (a :: TYPE WordRep). a -> Maybe# a
...
type RebindMaybe# :: forall (r :: RuntimeRep). TYPE r -> TYPE ('SumRep '[ 'TupleRep '[], WordRep ])
type family RebindMaybe# where
  RebindMaybe# @WordRep = Maybe# @WordRep
  -- no otherwise

type instance Rebind (Maybe# @WordRep) r = RebindMaybe# @r

instance Functor (Maybe# @WordRep) where
  type FunctorRep (Maybe# @WordRep) = (~) WordRep
  fmap _ Nothing# = Nothing#
  fmap f (Just# a) = Just# (f a)

instance Show a => Show (Maybe# @WordRep a) where
  showsPrec _ Nothing# = showString "Nothing#"
  showsPrec d (Just# a) = showParen (d >= 11) $ showString "Just# " . showsPrec 11 a
  show x = shows x ""
...
-- instance
instance (ListRep ('SumRep '[ 'TupleRep '[], WordRep ]), Show a) => ShowList (Maybe# @WordRep a) where
  showList = go shows where
    go :: forall (a :: TYPE WordRep). (Maybe# a -> ShowS) -> List (Maybe# a) -> ShowS
...
instance ShowRep WordRep where
  showsPrecDef _ x s = show x ++ s
  showDef x          = shows x ""

instance ShowListRep WordRep where
  showListDef = showList__ shows

showList__ :: forall (a :: TYPE WordRep). (a -> ShowS) -> List a -> ShowS
...
instance PrintRep WordRep where
