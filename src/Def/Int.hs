...
instance EqRep IntRep where
  eqDef x y = not (x /= y)
  neDef x y = not (x == y)

instance OrdRep IntRep where
  compareDef x y
    | x == y = EQ
    | x <= y = LT
    | otherwise = GT
...
instance FractionalRep IntRep where
  fractionalDef x y = x * recip y
  recipDef x = 1 / x

instance RealRep IntRep where
  realToFracDef x = fromRational (toRational x)

instance EnumRep IntRep where
{-
...
instance IntegralRep IntRep where
...
instance RealFracRep IntRep where
...
instance FloatingRep IntRep where
  {-# INLINE powDef #-}
  {-# INLINE logBaseDef #-}
  {-# INLINE sqrtDef #-}
  {-# INLINE tanDef #-}
...
instance RealFloatRep IntRep where
  exponentDef x
    | m == 0 = 0
...
data instance ListD (a :: TYPE IntRep) = Nil | a :# List a
infixr 5 :#

instance ListRep IntRep where
  cons = (:#) 
  cons' a as = a :# as
  nil = Nil
...
instance Eq a => Prelude.Eq (ListD @IntRep a) where
  Nil == Nil = True
  a :# as == b :# bs = a == b && as == bs
  _ == _ = False
...
instance Ord a => Prelude.Ord (ListD @IntRep a) where
  compare Nil Nil = EQ
  compare Nil (:#){} = LT
  compare (:#){} Nil = GT
  compare (a:#as) (b:#bs) = compare a b <> compare as bs

instance ShowList a => Prelude.Show (ListD @IntRep a) where
  showsPrec _ = showList

data instance MaybeD (a :: TYPE IntRep) = Nothing | Just a

instance Eq a => Prelude.Eq (MaybeD @IntRep a) where
  Nothing == Nothing = True
  Just a == Just b = a == b
  _ == _ = False
  
instance Ord a => Prelude.Ord (MaybeD @IntRep a) where
  compare Nothing  Nothing = EQ
  compare Nothing  Just{} = LT
  compare Just{}   Nothing = GT
  compare (Just a) (Just b) = compare a b

instance Show a => Prelude.Show (MaybeD @IntRep a) where
  showsPrec _ Nothing = showString "Nothing"
  showsPrec d (Just a) = showParen (d >= 11) $ showString "Just " . showsPrec 11 a

instance MaybeRep IntRep where
  nothing = Nothing
  just = Just
  just' x = Just x
  maybe n _ Nothing = n
...
pattern Nothing# :: forall (a :: TYPE IntRep). Maybe# a
pattern Nothing# = Maybe# (# (##) | #)

pattern Just# :: forall (a :: TYPE IntRep). a -> Maybe# a
...
type RebindMaybe# :: forall (r :: RuntimeRep). TYPE r -> TYPE ('SumRep '[ 'TupleRep '[], IntRep ])
type family RebindMaybe# where
  RebindMaybe# @IntRep = Maybe# @IntRep
  -- no otherwise

type instance Rebind (Maybe# @IntRep) r = RebindMaybe# @r

instance Functor (Maybe# @IntRep) where
  type FunctorRep (Maybe# @IntRep) = (~) IntRep
  fmap _ Nothing# = Nothing#
  fmap f (Just# a) = Just# (f a)

instance Show a => Show (Maybe# @IntRep a) where
  showsPrec _ Nothing# = showString "Nothing#"
  showsPrec d (Just# a) = showParen (d >= 11) $ showString "Just# " . showsPrec 11 a
  show x = shows x ""
...
-- instance
instance (ListRep ('SumRep '[ 'TupleRep '[], IntRep ]), Show a) => ShowList (Maybe# @IntRep a) where
  showList = go shows where
    go :: forall (a :: TYPE IntRep). (Maybe# a -> ShowS) -> List (Maybe# a) -> ShowS
...
instance ShowRep IntRep where
  showsPrecDef _ x s = show x ++ s
  showDef x          = shows x ""

instance ShowListRep IntRep where
  showListDef = showList__ shows

showList__ :: forall (a :: TYPE IntRep). (a -> ShowS) -> List a -> ShowS
...
instance PrintRep IntRep where
