...
instance STRep ('BoxedRep 'Lifted) where
  ...
  bindST :: forall (a :: TYPE ('BoxedRep 'Lifted)) rb (b :: TYPE rb) s. ST s a %1 -> (a %1 -> ST s b) %1 -> ST s b
  bindST f0 k0 = ST (\s -> toLinear3 go f0 k0 s) where
    go :: ST s a -> (a %1 -> ST s b) -> State# s -> (# State# s, b #)
    go f k s = case unST f s of
      (# s', a #) -> unST (k a) s'

  fmapST :: forall (a :: TYPE ('BoxedRep 'Lifted)) rb (b :: TYPE rb) s. STRep rb => (a %1 -> b) %1 -> ST s a %1 -> ST s b
  fmapST f0 m0 = ST (\s -> toLinear3 go f0 m0 s) where
    go :: (a %1 -> b) -> ST s a -> State# s -> (# State# s, b #)
    go f m s = case unST m s of
      (# s', a #) -> mkSTRes s' (f a)

  mkSTRes :: forall (b :: TYPE ('BoxedRep 'Lifted)) s. State# s -> Lev b -> (# State# s, b #)
  mkSTRes s a = (# s , a #)
